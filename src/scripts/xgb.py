...
def main(
    ...
    if gridsearch:
        try:
            model = GridSearchCV(xgb.XGBClassifier(**rparams), gparams, scoring=mcc, cv=n_folds, n_jobs=n_jobs, verbose=10)
            print("FIT")
            logging.info("FIT")
            model.fit(x_train, y_train.reshape(y_train.shape[0],))
        except:
            model = GridSearchCV(xgb.XGBClassifier(**rparams), gparams, cv=n_folds, n_jobs=n_jobs, verbose=10)
            print("FIT")
            logging.info("FIT")
            model.fit(x_train, y_train.reshape(y_train.shape[0],))
    else:
