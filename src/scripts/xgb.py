...
def main(
    ...
    if gridsearch:
        try:
            model = RandomizedSearchCV(xgb.XGBClassifier(**rparams), gparams, n_iter=n_iter, n_jobs=n_jobs, cv=n_folds, verbose=10, scoring=[mcc, 'f1_weighted', 'precision_weighted', 'r2', 'recall_weighted'])
            print("FIT")
            logging.info("FIT")
            model.fit(x_train, np.ravel(y_train))
        except:
            model = RandomizedSearchCV(xgb.XGBClassifier(**rparams), gparams, n_iter=n_iter, n_jobs=n_jobs, cv=n_folds, verbose=10)
            print("FIT")
            logging.info("FIT")
            model.fit(x_train, np.ravel(y_train))
    else:
    ...
    write_experiment(train_acc, test_acc, targets, experiments_file) # change folder name ?
