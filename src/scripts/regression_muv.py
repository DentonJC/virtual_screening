...
filename = os.path.dirname(os.path.realpath(__file__)).replace("/src", "").replace("/scripts", "") + "/data/muv_maccs.csv"
MACCS = True
Morgan = False
...
model = build_logistic_model(input_shape, output_shape, activation=rparams.get("activation", 'softmax'), loss=rparams.get("loss", 'binary_crossentropy'), metrics=['accuracy'], optimizer=rparams.get("_optimizer", 'adam'))    
