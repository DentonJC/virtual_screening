...
from src.configs.config_hiv import *
...
model = build_logistic_model(input_shape, output_shape, activation=rparams.get("activation", 'softmax'), loss=rparams.get("loss", 'binary_crossentropy'), metrics=['binary_accuracy'], optimizer=rparams.get("_optimizer", 'adam'))    
