...
if __name__ == "__main__":
    ...
    # Test molprint
    print(molprint2d_count_fingerprinter(np.array(["N(=O)O", "[N+](=O)(O)[O-]"])))

    # Split
    n_splits = 5

    X, y, smi, _ = bace(featurizer="smiles")
    X_molprint, _ = molprint2d_count_fingerprinter(np.array(smi))
    X_molprint = X_molprint.astype(np.float32)
    K_molprint_tanimoto = tanimoto_similarity(X_molprint, X_molprint)
    ...
    print(X_2d[0])
    plt.scatter(X_2d[np.where(labels==0)[0], 0], X_2d[np.where(labels==0)[0], 1], color="red")
    plt.scatter(X_2d[np.where(labels!=0)[0], 0], X_2d[np.where(labels!=0)[0], 1], color="black")
