...
# from keras.layers import TimeDistributed, SimpleRNN, RepeatVector, Wrapper
...
def create_callbacks(output, patience, section, monitor='val_acc', mode='auto',
    ...
    lr = ReduceLROnPlateau(monitor=monitor, factor=factor, patience=int(patience/3),
                           verbose=1, mode=mode, cooldown=0, min_lr=0)
    ...
    callbacks_list = []
    for i in ['stopping', 'csv_logger', 'checkpoint', 'remote', 'lr']:
        if i in callbacks:
            callbacks_list.append(all_callbacks[i])
